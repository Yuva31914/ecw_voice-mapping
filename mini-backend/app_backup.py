...
SCOPES        = os.getenv("SCOPES", "openid fhirUser offline_access user/Patient.read")
...
def debug_authorize_url():
    ...
    ver, chal = _pkce_pair()
    state = "DEBUG_ONLY_" + secrets.token_urlsafe(8)
...
"""
    ...
    try:
        # Try multiple Ollama endpoints with fallback models
        ollama_endpoints = [
            "http://108.192.20.12:11434/api/generate",  # Remote server
            "http://localhost:11434/api/generate",       # Local fallback
        ]
        models_to_try = ["llama3.3:latest", "llama4:latest", "llama3.2:3b"]  # Try fastest first
        
        raw = ""  # Initialize raw variable
        
        # Smart transcript truncation for long conversations (do this once)
        transcript_for_ai = transcript
        if len(transcript) > 8000:  # If transcript is very long
            print(f"Transcript is {len(transcript)} chars, applying smart truncation...")
            
            # Split into sentences and prioritize clinical content
            sentences = transcript.split('. ')
            
            # Priority keywords for clinical content
            clinical_keywords = [
                'pain', 'symptom', 'medication', 'drug', 'allergy', 'diagnosis', 
                'condition', 'treatment', 'prescription', 'dose', 'frequency',
                'blood pressure', 'diabetes', 'hypertension', 'heart', 'chest',
                'headache', 'fever', 'cough', 'nausea', 'vomiting', 'dizziness',
                'history', 'family history', 'medical history', 'surgical',
                'lab', 'test', 'result', 'procedure', 'follow', 'appointment'
            ]
            ...
            # Sort by clinical relevance (highest first)
            scored_sentences.sort(reverse=True)
            
            # Take most clinically relevant sentences up to 6000 chars
            selected_sentences = []
            current_length = 0
            for score, sentence in scored_sentences:
                if current_length + len(sentence) > 6000:
                    break
                selected_sentences.append(sentence)
                current_length += len(sentence)
            
            # Add beginning and end for context
            if len(transcript) > 6000:
                beginning = transcript[:1000]
                end = transcript[-1000:]
                transcript_for_ai = f"[BEGINNING OF CONVERSATION]\n{beginning}\n\n[MOST CLINICALLY RELEVANT PARTS]\n" + ". ".join(selected_sentences) + f"\n\n[END OF CONVERSATION]\n{end}"
            else:
                transcript_for_ai = ". ".join(selected_sentences)
            
            print(f"Truncated transcript to {len(transcript_for_ai)} chars (was {len(transcript)})")
        
        for ollama_url in ollama_endpoints:
            for ollama_model in models_to_try:
                print(f"Trying Ollama at {ollama_url} with model {ollama_model}")
                
                # Update prompt with truncated transcript
                prompt = f"""
You are an expert medical information extraction system. Your task is to thoroughly analyze this clinical conversation and extract ALL medical details into structured JSON format.
...
{{
  "summary": "comprehensive clinical summary",
  "diagnoses": [
    {{ "text": string, "code": string?, "severity": string? }}
  ],
  "medications": [
    {{ "name": string, "dose": string?, "route": string?, "frequency": string?, "duration": string?, "status": string? }}
  ],
  "allergies": [
    {{ "substance": string?, "reaction": string?, "severity": string?, "status": string? }}
  ],
  "labs": [
    {{ "name": string, "value": string?, "unit": string?, "date": string? }}
  ],
  "procedures": [
    {{ "name": string, "date": string? }}
  ],
  "tasks": [
    {{ "type": "followup" | "reminder" | "referral" | "message", "text": string, "due": string? }}
  ]
}}
...
"""
        ...
        payload = {
            ...
            "options": {
                ...
                "num_predict": 2000,  # Allow reasonable response size
                "top_p": 0.9,
                "top_k": 40,
            }
        }
                ...
                try:
                    ...
                    if raw:  # Success, break out of both loops
                        print(f"✓ Success with {ollama_url} + model {ollama_model}")
                        break
                except Exception as e:
                    print(f"✗ {ollama_url} + model {ollama_model} failed: {e}")
                    continue  # Try next model/endpoint
            else:
                continue  # Continue to next endpoint
            break  # Break out of endpoint loop if successful
        
        # Check if we got any response
        if not raw:
            print("All Ollama endpoints failed, using fallback data")
